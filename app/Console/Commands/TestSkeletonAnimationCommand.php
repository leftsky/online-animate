    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $this->table(
                ['属性', '值'],
                [
                    ['动作类型', $result['metadata']['action_type'] ?? '未知'],
                    ['置信度', number_format($result['metadata']['confidence'] ?? 0, 2)],
                    ['动画时长', ($result['animation_data']['duration'] ?? -1) . ' 秒'],
                    ['总帧数', $result['animation_data']['total_frames'] ?? 0],
            ...
            if (isset($result['metadata']['ai_analysis'])) {
                ...
                $this->table(
                    ['分析项', '结果'],
                    [
                        ['动作类型', $aiAnalysis['action_type'] ?? '未知'],
                        ['置信度', number_format($aiAnalysis['confidence'] ?? 0, 2)],
                        ['时长', ($aiAnalysis['duration'] ?? 0) . ' 秒'],
                        ['速度', number_format($aiAnalysis['speed'] ?? 0, 2)],
                        ['强度', number_format($aiAnalysis['intensity'] ?? 0, 2)],
                        ['方向', $aiAnalysis['direction'] ?? '未知'],
                        ['风格', $aiAnalysis['style'] ?? '默认'],
                    ]
                );
            ...
            if (isset($result['animation_data']['tracks'])) {
                ...
                foreach ($tracks as $index => $track) {
                    $this->newLine();
                    $trackNumber = $index + 1;
                    $this->line("轨道 {$trackNumber}: " . ($track['name'] ?? '未命名'));
            ...
            if (isset($result['animation_data']['tracks'])) {
                ...
                foreach ($tracks as $track) {
                    ...
                    if (count($track['times'] ?? []) < 2) {
                        $isCompatible = false;
                        $compatibilityIssues[] = "轨道 " . ($track['name'] ?? '未命名') . " 时间轴关键帧不足";
                    }
                    ...
                    if (isset($track['rotations'])) {
                        foreach (($track['rotations'] ?? []) as $rotation) {
                            if (!is_array($rotation) || count($rotation) !== 4) {
                                $isCompatible = false;
                                $compatibilityIssues[] = "轨道 " . ($track['name'] ?? '未命名') . " 旋转数据格式错误，需要4个值 [x,y,z,w]";
                                break;
                            }
                        }
                    }
                    ...
                    if (isset($track['positions'])) {
                        foreach (($track['positions'] ?? []) as $position) {
                            if (!is_array($position) || count($position) !== 3) {
                                $isCompatible = false;
                                $compatibilityIssues[] = "轨道 " . ($track['name'] ?? '未命名') . " 位置数据格式错误，需要3个值 [x,y,z]";
                                break;
                            }
                        }
                    }
