...
export class CanvasCore {
  ...
  constructor(canvasElement: HTMLCanvasElement | string, options?: CanvasOptions) {
  ...
  private updateAnimationEffect(obj: FabricObject, animation: AnimationEffect, progress: number): void {
    ...
    // 应用缓动函数
    const easedProgress = this.applyEasing(animationProgress, animation.easing || 'ease');
    
    // 根据动画类型应用效果
    switch (animation.type) {
      case 'move':
        this.applyMoveAnimation(obj, animation.properties, easedProgress);
        break;
      case 'scale':
        this.applyScaleAnimation(obj, animation.properties, easedProgress);
        break;
      case 'rotate':
        this.applyRotateAnimation(obj, animation.properties, easedProgress);
        break;
      case 'fade':
        this.applyFadeAnimation(obj, animation.properties, easedProgress);
        break;
      default:
        // 通用属性动画
   ...
   * 应用移动动画
   */
  private applyMoveAnimation(obj: FabricObject, properties: any, progress: number): void {
    if (properties.to) {
      const currentLeft = obj.left || 0;
      const currentTop = obj.top || 0;
      const targetLeft = properties.to.x || currentLeft;
      const targetTop = properties.to.y || currentTop;
      
      obj.set({
        left: currentLeft + (targetLeft - currentLeft) * progress,
        top: currentTop + (targetTop - currentTop) * progress
      });
    }
  }
   ...
   * 应用缩放动画
   */
  private applyScaleAnimation(obj: FabricObject, properties: any, progress: number): void {
    if (properties.to) {
      const currentScaleX = obj.scaleX || 1;
      const currentScaleY = obj.scaleY || 1;
      const targetScaleX = properties.to.scaleX || properties.to.scale || currentScaleX;
      const targetScaleY = properties.to.scaleY || properties.to.scale || currentScaleY;
      
      obj.set({
        scaleX: currentScaleX + (targetScaleX - currentScaleX) * progress,
        scaleY: currentScaleY + (targetScaleY - currentScaleY) * progress
      });
    }
  }
   ...
   * 应用旋转动画
   */
  private applyRotateAnimation(obj: FabricObject, properties: any, progress: number): void {
    if (properties.to) {
      const currentAngle = obj.angle || 0;
      const targetAngle = properties.to.rotation || properties.to.angle || currentAngle;
      
      obj.set({
        angle: currentAngle + (targetAngle - currentAngle) * progress
      });
    }
  }
   ...
   * 应用淡入淡出动画
   */
  private applyFadeAnimation(obj: FabricObject, properties: any, progress: number): void {
    if (properties.to) {
      const currentOpacity = obj.opacity || 1;
      const targetOpacity = properties.to.opacity || currentOpacity;
      
      obj.set({
        opacity: currentOpacity + (targetOpacity - currentOpacity) * progress
      });
    }
  }
  ...
  public setPlaybackSpeed(speed: number): void {
    // 调整时间计算来实现变速播放
    const currentProgress = this.currentTime / this.totalDuration;
    this.totalDuration = this.totalDuration / speed;
    this.currentTime = currentProgress * this.totalDuration;
    this.startTime = performance.now() - this.currentTime;
  }
}
