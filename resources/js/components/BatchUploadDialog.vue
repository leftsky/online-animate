    ...
    <Dialog :open="open" @update:open="(value) => emit('update:open', value)">
        <DialogContent class="max-w-2xl">
            <DialogHeader>
                ...
                <DialogDescription> 拖拽图片文件到此处，将自动创建{{ resourceTypeText }}资源 </DialogDescription>
            </DialogHeader>
            ...
            <div class="space-y-4">
                ...
                <div
                    ref="dropZone"
                    class="rounded-lg border-2 border-dashed border-border p-8 text-center transition-colors"
                    :class="{
                ...
                >
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-muted-foreground">支持 JPG、PNG、GIF、WebP 格式，单个文件最大 50MB</p>
                        </div>
                        <Button variant="outline" size="sm" @click.stop="triggerFileInput"> 选择文件上传 </Button>
                    </div>
                </div>

                <!-- 文件输入 -->
                <input ref="fileInput" type="file" multiple accept=".jpg,.jpeg,.png,.gif,.webp" class="hidden" @change="handleFileSelect" />

                <!-- 上传进度 -->
                <div v-if="uploadingFiles.length > 0" class="space-y-2">
                    <h4 class="text-sm font-medium">上传进度</h4>
                    <div class="max-h-40 space-y-2 overflow-y-auto">
                        <div v-for="file in uploadingFiles" :key="file.id" class="flex items-center justify-between rounded bg-muted p-2">
                            <div class="flex min-w-0 flex-1 items-center space-x-2">
                                <div class="flex h-8 w-8 items-center justify-center rounded bg-background">
                            ...
                            <div class="flex items-center space-x-2">
                                <div v-if="file.status === 'uploading'" class="text-xs text-muted-foreground">{{ file.progress }}%</div>
                                <div v-else-if="file.status === 'success'" class="text-xs text-green-600">成功</div>
                                <div v-else-if="file.status === 'error'" class="text-xs text-red-600">失败</div>
                ...
                <div v-if="uploadResults.length > 0" class="space-y-2">
                    ...
                    <div class="max-h-40 space-y-2 overflow-y-auto">
                        <div v-for="result in uploadResults" :key="result.id" class="flex items-center justify-between rounded bg-muted p-2">
                            <div class="flex min-w-0 flex-1 items-center space-x-2">
                                <div class="flex h-8 w-8 items-center justify-center rounded bg-background">
                                    <Check v-if="result.success" class="h-4 w-4 text-green-600" />
            ...
            <DialogFooter>
                ...
                <Button v-if="uploadingFiles.length > 0" @click="retryFailedUploads" :disabled="!hasFailedUploads"> 重试失败 </Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
</template>
...
import { Check, Image, Upload, X } from 'lucide-vue-next';
