...
import { Code, Image, Play, Plus } from 'lucide-vue-next';
...
import type { ApiSceneContent, StoryboardItem as StoryboardItemType } from './types';
...
const selectedItemAnimations = computed(() => {
    if (!selectedItem.value?.animationScript) return [];
    try {
...
const getInitialPosition = () => {
    ...
    try {
        const parsedData = AnimationParser.parseYamlToJson(selectedItem.value.animationScript);
        return {
            x: Number(parsedData.initialPosition?.x) || 0,
            y: Number(parsedData.initialPosition?.y) || 0,
            scaleX: Number(parsedData.initialPosition?.scaleX) || 1,
            scaleY: Number(parsedData.initialPosition?.scaleY) || 1,
            opacity: Number(parsedData.initialPosition?.opacity) || 1,
        };
    } catch {
        return { x: 0, y: 0, scaleX: 1, scaleY: 1, opacity: 1 };
    }
...
const loadStoryboardData = async () => {
    ...
    try {
        ...
        if (response.success && response.data) {
            ...
            // 按layer_order降序排列，数字大的在上面
            storyboardItems.value.sort((a, b) => b.layerOrder - a.layerOrder);
        } else {
...
const handleSelectedItemViewSource = () => selectedItem.value && sourceCodeViewer.value?.open(selectedItem.value);
const handleSelectedItemPreviewAnimation = () => selectedItem.value && emit('previewAnimation', selectedItem.value);
...
const handlePreviewAnimation = (item: StoryboardItemType) => emit('previewAnimation', item);

// 处理动画管理（临时使用预览动画替代）
const handleManageAnimations = (item: StoryboardItemType) => emit('previewAnimation', item);
...
// 添加新项目
const addNewItem = () => !loading.value && createContentDialog.value?.open();
...
const totalDuration = computed(() => {
    return storyboardItems.value.reduce((total, item) => {
        ...
        try {
            ...
            if (animationData.animationSequences && animationData.animationSequences.length > 0) {
                const itemDuration = animationData.animationSequences.reduce((sum, seq) => sum + (seq.duration || 0), 0);
...
const savePositionChanges = async () => {
    ...
    try {
        ...
        const hasChanges = Object.keys(newPosition).some(
            (key) => newPosition[key as keyof typeof newPosition] !== originalPosition[key as keyof typeof originalPosition],
        );
        ...
        if (animationData.initialPosition) {
            Object.assign(animationData.initialPosition, newPosition);
            const newAnimationScript = AnimationParser.parseJsonToYaml(animationData);
            const updatedItem = { ...selectedItem.value, animationScript: newAnimationScript };

            await submitToApi(updatedItem);
            emit('updateItem', updatedItem);

            await nextTick();
            editingPosition.value = { ...getInitialPosition() };
    ...
    <div class="flex h-full flex-col bg-card">
        <div class="flex items-center justify-between border-b p-3">
            ...
            <Button
                ...
                @click="addNewItem"
                :disabled="loading"
                class="flex items-center rounded-md bg-primary px-3 py-1.5 text-sm text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
                title="添加新动画"
            >
        ...
        <div class="flex items-center justify-between border-b bg-muted/20 p-3">
            ...
            <Button
                ...
                class="h-7 border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 px-2 text-xs text-green-700 transition-all duration-200 hover:border-green-300 hover:from-green-100 hover:to-emerald-100"
        ...
        <div class="flex-1 overflow-y-auto">
            <VueDraggable v-model="storyboardItems" :animation="200" handle=".drag-handle" class="space-y-2 p-3" @end="onDragEnd">
                <StoryboardItem
            ...
            <div v-if="storyboardItems.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
                ...
                <button
                    @click="addNewItem"
                    :disabled="loading"
                    class="mt-4 flex items-center rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
                >
        ...
        <div v-if="selectedItem" class="space-y-2 border-t bg-muted/20 p-3">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <input
                        ...
                        type="text"
                        class="rounded border border-input bg-background px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                        @keydown.enter="confirmTitleEdit"
                        @keydown.esc="cancelTitleEdit"
                        @blur="confirmTitleEdit"
                    />
                    <h4 v-else class="cursor-pointer text-sm font-medium transition-colors hover:text-primary" @click="startTitleEdit">
            ...
            <div class="space-y-2">
                <div class="grid grid-cols-3 gap-2 text-xs">
                    <div class="rounded border bg-background p-2">
                        <div class="mb-1 font-medium text-muted-foreground">初始位置</div>
                        <div class="space-y-1">
                            <div class="flex items-center gap-1">
                                ...
                                <input
                                    ...
                                    type="number"
                                    class="w-full rounded border border-input bg-transparent px-1 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-ring"
                                    @blur="savePositionChanges"
                                    @keydown.enter="savePositionChanges"
                                />
                            </div>
                            <div class="flex items-center gap-1">
                    ...
                    <div class="rounded border bg-background p-2">
                        <div class="mb-1 font-medium text-muted-foreground">缩放</div>
                        <div class="space-y-1">
                            <div class="flex items-center gap-1">
                                <span class="w-3">X:</span>
                                <input
                                    v-model="editingPosition.scaleX"
                                    type="number"
                                    step="0.1"
                                    min="0.1"
                                    max="5"
                            ...
                            <div class="flex items-center gap-1">
                                <span class="w-3">Y:</span>
                                <input
                                    v-model="editingPosition.scaleY"
                                    type="number"
                                    step="0.1"
                                    min="0.1"
                                    max="5"
                    ...
                    <div class="rounded border bg-background p-2">
                        <div class="mb-1 font-medium text-muted-foreground">透明度</div>
                        <div class="flex items-center gap-1">
                            <input
                                v-model="editingPosition.opacity"
                                type="number"
                                step="0.1"
                                min="0"
                                max="1"
            ...
            <div class="flex items-center gap-1 border-t border-border pt-2">
                <Button variant="outline" size="sm" @click="handleSelectedItemViewSource" class="h-7 px-2 text-xs">
        ...
        >
          ...
          管理动画
                ...
                <Button
                    variant="outline"
                    size="sm"
                    @click="handleSelectedItemPreviewAnimation"
                    class="h-7 transform border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 px-2 text-xs text-blue-700 transition-all duration-200 hover:scale-105 hover:border-blue-300 hover:from-blue-100 hover:to-indigo-100 active:scale-95"
                >
                    <Play class="mr-1 h-3 w-3 animate-pulse" />
                    预览动画
                </Button>
            </div>
        </div>
