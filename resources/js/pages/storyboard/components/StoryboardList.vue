...
const selectedItemAnimations = computed(() => {
  ...
  try {
    const parsedData = AnimationParser.parseYamlToJson(
      selectedItem.value.animationScript
    );
...
const handleSelectedItemPreviewAnimation = () =>
  selectedItem.value && emit("previewAnimation", selectedItem.value);
...
const handleAnimationSaved = (updatedItem: StoryboardItemType) => {
  ...
  if (index > -1) {
    ...
    storyboardItems.value[index].duration = parseDurationToString(
      updatedItem.animationScript
    );
  }
};
...
const totalDuration = computed(() => {
  return storyboardItems.value.reduce((total, item) => {
    ...
    try {
      ...
      if (
        animationData.animationSequences &&
        animationData.animationSequences.length > 0
      ) {
        const itemDuration = animationData.animationSequences.reduce(
          (sum, seq) => sum + (seq.duration || 0),
          0
        );
...
const savePositionChanges = async () => {
  ...
  try {
    ...
    const hasChanges = Object.keys(newPosition).some(
      (key) =>
        newPosition[key as keyof typeof newPosition] !==
        originalPosition[key as keyof typeof originalPosition]
    );

    if (!hasChanges) return;

    const animationData = AnimationParser.parseYamlToJson(
      selectedItem.value.animationScript
    );
    ...
    <div v-if="selectedItem" class="border-t bg-muted/20 p-3 space-y-2">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          >
            {{ selectedItem.elementName }} - 动画效果 ({{
              selectedItemAnimations.length
            }}个动画 • {{ selectedItem.duration }})
          </h4>
        </div>
      </div>
      ...
      <div class="flex items-center gap-1 pt-2 border-t border-border">
        <Button
          variant="outline"
          size="sm"
          @click="handleSelectedItemViewSource"
