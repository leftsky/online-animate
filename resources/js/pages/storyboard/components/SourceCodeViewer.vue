    ...
    <DialogContent class="max-w-4xl max-h-[90vh]">
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            <pre class="bg-muted p-4 rounded-lg text-sm overflow-auto max-h-96 border"><code>{{ formattedScript }}</code></pre>
            
            <!-- 空状态 -->
            <div v-if="!item?.animationScript" class="absolute inset-0 flex items-center justify-center bg-muted/50 rounded-lg">
              <div class="text-center text-muted-foreground">
                <Code class="w-8 h-8 mx-auto mb-2" />
        ...
        <div v-if="item?.imagePath" class="space-y-2">
          ...
          <div class="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium truncate">{{ getFileName(item.imagePath) }}</p>
